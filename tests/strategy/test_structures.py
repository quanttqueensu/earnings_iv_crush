...
from src.engine.greeks import bs_vega, straddle_price
from src.strategy import regime, structures
...
def test_iron_fly_credit_below_naked_straddle():
    ...
    fly = structures.iron_fly_pnl(
        ...
        iv_entry=0.5, iv_exit=0.3, spot_exit=100.0, implied_move=0.06,
    )
...
def test_iron_fly_loss_is_capped_at_expiry():
    ...
    fly = structures.iron_fly_pnl(
        spot_entry=100, strike=100, t_entry=0.05, t_exit=0.0,
        iv_entry=0.5, iv_exit=0.5, spot_exit=200.0, implied_move=0.06,
    )
...
def test_calendar_ratio_is_vega_balanced():
    ratio = structures.calendar_ratio(100, 100, t_front=0.02, t_back=0.12,
                                      iv_front=0.5, iv_back=0.4)
...
def test_calendar_profits_when_front_crushes_more():
    cal = structures.calendar_pnl(
        spot_entry=100, strike=100,
        t_front_entry=0.02, t_front_exit=0.005,
        t_back_entry=0.12, t_back_exit=0.105,
        iv_front_entry=0.60, iv_front_exit=0.25,
        iv_back_entry=0.40, iv_back_exit=0.38,
        spot_exit=100.0,
    )
    assert cal["pnl"] > 0
