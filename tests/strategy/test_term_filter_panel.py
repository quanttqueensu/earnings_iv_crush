...
from src.strategy.filters import passes_term_filter_panel, select_events


def _panel(ticker, dates, spreads):
    return pd.DataFrame({"ticker": ticker, "date": pd.to_datetime(dates),
                         "iv_term_spread": spreads})
...
def test_too_few_obs_rejected():
    ...
    events = pd.DataFrame({"ticker": ["AAA"], "announce_date": ["2024-06-10"],
                           "iv_term_spread": [0.99]})
...
def test_select_events_uses_panel_when_supplied():
    ...
    events = pd.DataFrame({
        "ticker": ["AAA"], "announce_date": ["2024-06-10"],
        "implied_move": [0.06], "iv_term_spread": [0.30],
    })
