...
def _bs_smile_chain(expiry, spot, t, slope):
    ...
    for k in range(70, 131, 5):
        ...
        for right in ("C", "P"):
            ...
            rows.append({"expiry": pd.Timestamp(expiry), "strike": float(k),
                         "right": right, "bid": px, "ask": px, "iv": iv,
                         "open_interest": 100})
...
def test_bkm_thin_chain_is_nan():
    ...
    for k in (95, 105):
        ...
        rows.append({"expiry": pd.Timestamp("2026-06-05"), "strike": float(k),
                     "right": right, "bid": 1.0, "ask": 1.0, "iv": 0.4,
                     "open_interest": 1})
...
def test_fair_move_model_graceful_without_new_features():
    ...
    ev = pd.DataFrame({"trailing_rv": rng.uniform(0.2, 0.8, 80),
                       "skew_25d": rng.uniform(-0.1, 0.1, 80)})
