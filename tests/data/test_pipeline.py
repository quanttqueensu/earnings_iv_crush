"""Tests for earnings_iv_crush.data.data_pipeline.build_event_dataset.
...
from earnings_iv_crush.data import data_pipeline
...
def _no_dispersion(ticker, start, end):
    return pd.DataFrame(
        columns=["ticker", "announce_date", "eps_mean", "eps_std", "n_estimates", "source"]
    )


def _fake_dispersion(ticker, start, end):
    return pd.DataFrame(
        [
            {
                "ticker": ticker,
...
def test_build_event_dataset_happy_path():
    df = data_pipeline.build_event_dataset(
        ...
        "2026-06-05",
        calendar=_calendar(),
        fetch_chain=_fake_chain,
        fetch_prices=_fake_prices,
        fetch_dispersion=_no_dispersion,
    )
    ...
    assert (
        df.loc[df["ticker"] == "AAPL", "implied_move"].iloc[0]
...
def test_eps_dispersion_populated_from_fetcher():
    df = data_pipeline.build_event_dataset(
        "2026-06-01",
        "2026-06-05",
        calendar=_calendar(),
        fetch_chain=_fake_chain,
        fetch_prices=_fake_prices,
        fetch_dispersion=_fake_dispersion,
    )
    # Normalised dispersion = eps_std / |eps_mean| = 0.1 / 2.0.
    assert np.allclose(df["eps_dispersion"], 0.05)


def test_oi_proxy_only_populates_behind_flag():
    kwargs = dict(
        calendar=_calendar(),
        fetch_chain=_fake_chain,
        fetch_prices=lambda t, s, e: _fake_prices(t, s, e).assign(volume=1_000_000),
        fetch_dispersion=_no_dispersion,
    )
    off = data_pipeline.build_event_dataset("2026-06-01", "2026-06-05", **kwargs)
    on = data_pipeline.build_event_dataset("2026-06-01", "2026-06-05", use_oi_proxy=True, **kwargs)
    assert off["oi_growth"].isna().all()
    assert on["oi_growth"].notna().all()
    # 12 contracts x 100 OI x 100 multiplier / 1m avg volume = 0.12.
    assert np.allclose(on["oi_growth"], 0.12)
