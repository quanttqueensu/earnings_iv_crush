"""Tests for src.data.term_panel.build_term_panel (offline, injected providers)."""
from __future__ import annotations

import numpy as np
import pandas as pd

from src.data import term_panel as tp
...
def test_builds_daily_rows_for_trailing_window():
    ...
    panel = tp.build_term_panel(
        events, fetch_chain=lambda t, d: _chain(d, 100.0, front_extra=0.2),
        fetch_prices=lambda t, s, e: _prices(t, s, e),
        window_days=20, asof_offset_days=1)
...
def test_empty_events_returns_typed_frame():
    panel = tp.build_term_panel(pd.DataFrame(columns=["ticker", "announce_date"]),
...
def test_dedupes_overlapping_windows_per_ticker():
    ...
    events = pd.DataFrame({"ticker": ["AAA", "AAA"],
                           "announce_date": ["2024-06-13", "2024-06-14"]})
    panel = tp.build_term_panel(
        events, fetch_chain=lambda t, d: _chain(d, 100.0, front_extra=0.2),
        fetch_prices=lambda t, s, e: _prices(t, s, e),
        window_days=20, asof_offset_days=1)
