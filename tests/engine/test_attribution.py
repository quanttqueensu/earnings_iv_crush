...
from earnings_iv_crush.engine import greeks
from earnings_iv_crush.engine.attribution import attribute_straddle_pnl, delta_hedge_pnl
...
def test_vega_matches_finite_difference():
    ...
    fd = (greeks.bs_price(s, k, t, r, sig + h, "C") - greeks.bs_price(s, k, t, r, sig - h, "C")) / (
        2 * h
    )
...
def test_gamma_matches_delta_finite_difference():
    ...
    fd = (greeks.bs_delta(s + h, k, t, r, sig, "C") - greeks.bs_delta(s - h, k, t, r, sig, "C")) / (
        2 * h
    )
...
def test_components_plus_residual_reconstruct_total():
    ...
    recon = a["delta_pnl"] + a["gamma_pnl"] + a["vega_pnl"] + a["theta_pnl"] + a["residual"]
