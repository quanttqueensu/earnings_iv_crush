...
class FairMoveModel:
    ...
    def _usable_features(self, events: pd.DataFrame) -> list[str]:
        ...
        return [c for c in self.features if c in events.columns and events[c].notna().any()]
    ...
    def predict(self, events: pd.DataFrame) -> pd.Series:
        ...
        if self.method == "ridge":
            ...
            pred = np.asarray(
                self.result.predict(sm.add_constant(X, has_constant="add")), dtype=float
            )
    ...
    def diagnostics(self) -> dict:
        ...
        params.update(
            {c: float(b) for c, b in zip(self.used_features, self._sk.coef_, strict=True)}
        )
        return {
            "method": "ridge",
            "n_obs": None,
    ...
    def fit_predict_walk_forward(
        self, events: pd.DataFrame, realised_move, min_train: int = 20
    ) -> pd.Series:
        ...
        for i in range(len(events)):
            ...
            train = events.iloc[:i]
            try:
                model = FairMoveModel(self.features, self.method, self.alpha).fit(train, y.iloc[:i])
            except ValueError:
                continue
