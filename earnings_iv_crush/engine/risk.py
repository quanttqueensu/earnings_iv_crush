...
Implements the risk rules that are testable without live data: size each
...
def worst_case_size(
    ...
    """
    worst_ps = (
        defined_max_loss_per_share
        if defined_max_loss_per_share is not None
...
def halt_new_entries(
    trades: pd.DataFrame, account: float, threshold: float = DRAWDOWN_CIRCUIT_BREAKER
) -> pd.DataFrame:
    """
