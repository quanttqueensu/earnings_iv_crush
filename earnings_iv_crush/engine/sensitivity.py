...
def threshold_sweep(
    ...
    agent0_sharpe = backtest(
        run_agent0(events, seed=agent0_seed, account=account, fraction=fraction, r=r, costs=costs),
        account,
    )["sharpe"]
    ...
    for ratio in ratios:
        for pctl in pctls:
            ...
            stats = backtest(
                build_ledger(selected, account=account, fraction=fraction, r=r, costs=costs),
                account,
            )
            rows.append(
                {
