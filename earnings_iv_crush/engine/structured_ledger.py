...
def build_structured_ledger(
    ...
    back_crush: float = DEFAULT_BACK_CRUSH,
) -> pd.DataFrame:
    """
    ...
    """
    rows = []
    for (_, e), label in zip(events.iterrows(), list(structures), strict=True):
...
def _book_straddle(
    e, spot, strike, t_entry, t_exit, iv_entry, iv_exit, spot_exit, account, risk_frac, r, costs
):
    ...
    pnl = straddle_pnl(
        spot, strike, t_entry, t_exit, iv_entry, iv_exit, spot_exit, r, contracts, costs=costs
    )
    ...
    return _common(e, "straddle", contracts, entry_credit, exit_value, cost, pnl, capital_at_risk)
...
def _book_iron_fly(
    ...
    contracts = worst_case_size(
        account, credit_ps, risk_frac=risk_frac, defined_max_loss_per_share=max_loss_ps
    )
    if contracts <= 0:
        return None
    res = iron_fly_pnl(
    ...
    pnl = res["pnl"] - cost
    return _common(
        e, IRON_FLY, contracts, res["entry_credit"], res["exit_value"], cost, pnl, res["max_loss"]
    )
...
def _book_calendar(
    ...
    return _common(
        e, CALENDAR, contracts, res["entry_credit"], res["exit_value"], cost, pnl, capital_at_risk
    )


def _structure_cost(
    entry_credit: float, exit_value: float, contracts: int, costs: CostModel | None
) -> float:
