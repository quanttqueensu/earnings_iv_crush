...
Black-Scholes inversion (``engine.greeks.implied_vol``).
...
_TIMEOUT = 30
_MAX_RETRIES = 6  # transient network/5xx/429 retries before giving up
_RETRY_BACKOFF = 2.0  # seconds, multiplied by the attempt number
_RATE_LIMIT_BACKOFF = 10.0  # seconds per attempt on 429 (sustained pressure)
...
def _get(host: str, path: str, params: dict) -> dict:
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        rate_limited = False
        try:
            r = requests.get(f"{host}{path}", params=params, headers=_headers(), timeout=_TIMEOUT)
        except (requests.ConnectionError, requests.Timeout) as exc:
            last_error = exc
        else:
            if r.status_code == 429:  # rate limited: long back-off and retry
                last_error = requests.HTTPError(f"429 rate limited: {path}")
                rate_limited = True
            elif r.status_code < 500:
                r.raise_for_status()  # other 4xx -> immediate (callers may catch)
                return r.json()
            else:
                last_error = requests.HTTPError(f"{r.status_code} server error: {path}")
        if attempt < _MAX_RETRIES:
            backoff = _RATE_LIMIT_BACKOFF if rate_limited else _RETRY_BACKOFF
...
def list_contracts(
    ...
    base = {
        "underlying_symbols": _alpaca_symbol(underlying),
            ...
            for c in body.get("option_contracts") or []:
                ...
                if sym and sym[0].isalpha() and sym not in seen:
                    seen[sym] = {
                        ...
                        "open_interest": pd.to_numeric(c.get("open_interest"), errors="coerce"),
                    }
...
def _alpaca_symbol(ticker: str) -> str:
    """Translate Yahoo-style class shares (BRK-B) to Alpaca's dot form (BRK.B)."""
...
def fetch_underlying_ohlcv(ticker: str, start: str, end: str) -> pd.DataFrame:
    """Daily OHLCV for the underlying from Alpaca IEX bars.

    A drop-in for ``equities.fetch_equity_ohlcv`` (same ``date``/``open``/
    ``high``/``low``/``close``/``volume`` schema) but sourced from Alpaca rather
    than yfinance, so the event assembler's price pulls are fast and share the
    one keyed session. ``end`` is inclusive. Empty (correctly typed) when no
    bars resolve.

    Parameters
    ----------
    ticker : str
        Underlying ticker (Yahoo-style class shares are translated to dot form).
    start, end : str
    ...
    sym = _alpaca_symbol(ticker)
    # Alpaca's bar window is half-open on the end; nudge it a day so `end` is
    # inclusive, matching the yfinance fetcher this stands in for.
    ...
    while True:
        ...
        body = _get(_DATA_HOST, "/v2/stocks/bars", params)
        bars.extend((body.get("bars") or {}).get(sym) or [])
