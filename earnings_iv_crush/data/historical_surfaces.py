...
def build_surface_panel(tickers, dates, fetch_chain, spot_lookup, r: float = 0.0) -> pd.DataFrame:
    """
    ...
    for ticker in tickers:
        for date in dates:
            ...
            k_back = (
                features.nearest_strike(chain, back, spot) if back is not None else features.NAN
            )
            ...
            spread = (
                front_iv - back_iv
                if (front_iv == front_iv and back_iv == back_iv)
                else features.NAN
            )
...
def join_earnings_to_surfaces(
    calendar: pd.DataFrame, panel: pd.DataFrame, realised_move_fn, asof_offset_days: int = 1
) -> pd.DataFrame:
    """
