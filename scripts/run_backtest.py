...
from src.engine.backtester import backtest                 # noqa: E402
...
KEYS = ["n_trades", "total_pnl", "total_return", "hit_rate", "sharpe",
        "max_drawdown", "avg_return_on_margin", "final_equity"]
...
def main() -> None:
    ...
    print(f"Simulated {len(events)} events; {int(events['is_rich'].sum())} rich "
          f"(planted edge).")
    ...
    print("Gate to justify the filter: >= +0.50 Sharpe.  "
