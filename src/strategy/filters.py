...
def passes_term_filter_panel(events: pd.DataFrame, panel: pd.DataFrame,
                             pctl=TERM_SPREAD_PCTL, window_days=TRAILING_WINDOW,
                             min_periods=TERM_MIN_PERIODS,
                             asof_offset_days=1) -> pd.Series:
    """Per-name trailing-DAY percentile gate (the spec's "30-day distribution").
...
def select_events(events: pd.DataFrame, fair_move,
                  ...
                  window_days=TRAILING_WINDOW, min_periods=TERM_MIN_PERIODS,
    ...
    if term_panel is not None:
        term_ok = passes_term_filter_panel(events, term_panel, pctl, window_days,
                                            min_periods, asof_offset_days)
    else:
