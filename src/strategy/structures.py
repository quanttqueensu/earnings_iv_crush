...
The naked short ATM straddle is the primary structure, but the spec (§2) calls
for two variants the regime selector switches to:
...
def iron_fly_pnl(spot_entry: float, strike: float, t_entry: float, t_exit: float,
                 iv_entry: float, iv_exit: float, spot_exit: float,
                 implied_move: float, r: float = 0.0, contracts: int = 1,
                 wing_mult: float = 1.5, multiplier: int = 100) -> dict:
    """
    ...
    wings_entry = (bs_price(spot_entry, upper, t_entry, r, iv_entry, "C")
                   + bs_price(spot_entry, lower, t_entry, r, iv_entry, "P"))
    ...
    body_exit = (_val(spot_exit, strike, t_exit, iv_exit, "C")
                 + _val(spot_exit, strike, t_exit, iv_exit, "P"))
    wings_exit = (_val(spot_exit, upper, t_exit, iv_exit, "C")
                  + _val(spot_exit, lower, t_exit, iv_exit, "P"))
...
def calendar_ratio(spot: float, strike: float, t_front: float, t_back: float,
                   iv_front: float, iv_back: float, r: float = 0.0) -> float:
    """
...
def calendar_pnl(spot_entry: float, strike: float,
                 t_front_entry: float, t_front_exit: float,
                 t_back_entry: float, t_back_exit: float,
                 iv_front_entry: float, iv_front_exit: float,
                 iv_back_entry: float, iv_back_exit: float,
    ...
    """
    ratio = calendar_ratio(spot_entry, strike, t_front_entry, t_back_entry,
                           iv_front_entry, iv_back_entry, r)
    scale = multiplier * contracts
