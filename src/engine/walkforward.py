...
The 31 July validation milestone needs an out-of-sample result, not the
in-sample fit. This runs the strategy so that every fair-move prediction is made
...
from ..strategy.filters import (
    IMPLIED_FAIR_RATIO, TERM_SPREAD_PCTL, TRAILING_WINDOW, select_events,
)
from .backtester import backtest
from .pnl import ACCOUNT_SIZE, build_ledger


def walk_forward_backtest(events: pd.DataFrame, realised_move,
                          model: FairMoveModel | None = None, min_train: int = 20,
                          ...
                          window: int = TRAILING_WINDOW) -> tuple[dict, pd.DataFrame]:
    """
