...
def aggregate_pnl_attribution(ledger: pd.DataFrame, r: float = 0.0) -> dict:
    ...
    for _, row in ledger.iterrows():
        a = attribute_straddle_pnl(
            spot_entry=float(row["spot_entry"]), strike=float(row["strike"]),
            t_entry=float(row["t_entry"]), t_exit=float(row["t_exit"]),
            iv_entry=float(row["iv_entry"]), iv_exit=float(row["iv_exit"]),
            spot_exit=float(row["spot_exit"]), r=r,
...
def build_tearsheet(strategy_ledger: pd.DataFrame, agent0_ledger: pd.DataFrame,
                    comparison: dict, account: float, outdir: str | Path,
    ...
    ax = axes[1, 1]
    if structure_counts:
        ax.bar(list(structure_counts.keys()), list(structure_counts.values()),
               color="slateblue")
